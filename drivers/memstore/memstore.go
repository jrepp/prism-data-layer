...
func (m *MemStore) Metadata() *core.BackendInterfaceMetadata {
	return &core.BackendInterfaceMetadata{
		...
		Interfaces: []core.BackendInterface{
			// KeyValue interfaces (2 of 6) - intentionally minimal for fast local testing
			core.InterfaceKeyValueBasic, // Set, Get, Delete, Exists
			core.InterfaceKeyValueTTL,   // TTL support with time.AfterFunc cleanup

			// Future: Add List interfaces for FIFO/LIFO operations
			// core.InterfaceListBasic,
			// core.InterfaceListIndexing,
			// core.InterfaceListRange,
			// core.InterfaceListBlocking,
		},
		Description:            "In-memory Go map for local testing (no persistence)",
